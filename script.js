...
// Fecha se o usuário clicar na área cinza (overlay) OU no botão de fechar (x)
function closeModal(event, modalId) {
  if (event) {
    // Se o clique foi no fundo (overlay) OU no botão de fechar/elemento interno do botão
    if (
      event.target.classList.contains('modal-overlay') || 
      event.target.classList.contains('modal-close-btn') ||
      event.currentTarget.classList.contains('modal-close-btn')
    ) {
